...
CS261 Assignment 4
3/7/18
This includes the implementations of the heap with a dynamic array*/
#include <assert.h>
#include <stdlib.h>
#include "dynArray.h"
...
{
	{
		...
		if (rightChild < max)		//two children
		{
			...
			if (dyArrayGet(heap, smallestChild) < dyarrayGet(heap, pos))
			{
				swap(heap, pos, smallestChild);
				adjustHeap(heap, max, smallestChild);	//inserted value now at index                                                                                                        //									//”smallestChild"
			}
		}
		else if (leftChild < max)		//one child
		{
			if (dyArrayGet(heap, leftChild) < dyArrayGet(heap, pos))
			{
